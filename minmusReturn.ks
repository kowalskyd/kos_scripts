...
if ship:body:name <> "Minmus" {
  clearScreen.
  print "=======================================".
  print "     MINMUS RETURN SCRIPT ERROR        ".
  print "=======================================".
  ...
  print "Run minmusLaunch first.".
  print "=======================================".
  wait 5.
} else {
  runOncePath("0:/lib/hud.ks").
  initScreen("minmus_return").
  ...
  // Calculate escape velocity from current Minmus orbit
  ...
  // -------------------------------------------
  if ship:body:name = "Minmus" {
    logChatter("CapCom", "Step 3: Coasting out of Minmus's sphere of influence...").
    lock steering to prograde.
    set MAPVIEW to true.
  ...
  logChatter("KAL-9000", "Deploying and arming parachutes...").
  hudMsg("PARACHUTES DEPLOYED").
  stage.
  playChuteScene().
  wait 0.5.
  chutes on.

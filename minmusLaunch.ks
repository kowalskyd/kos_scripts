...
local targetOrbit is 15000.

if ship:body:name <> "Minmus" {
  clearScreen.
  print "=======================================".
  print "     MINMUS LAUNCH SCRIPT ERROR        ".
  print "=======================================".
  print "Error: Vessel is not on Minmus.".
  ...
  print "     MINMUS LAUNCH SCRIPT ERROR        ".
  print "=======================================".
  print "Error: Vessel is not landed on Minmus.".
  print "Status: " + ship:status.
  print "=======================================".
  wait 5.
} else {
  runOncePath("0:/lib/hud.ks").
  initScreen("minmus_ascent").
  logChatter("CapCom", "Minmus launch sequence initiated.").
  hudMsg("MINMUS ASCENT PREPARATION").
  ...
  logChatter("CapCom", "Local TWR: " + round(twrMinmus, 2)).
  if twrMinmus < 1.5 {
  ...
  until alt:radar > 100 { // Minmus gravity and hills are smaller
  ...
  if ship:altitude < 3000 {
    logChatter("CapCom", "Coasting to safe warp altitude (3000m)...").
  ...
  logChatter("CapCom", "Mission summary: Minmus orbit established.").
